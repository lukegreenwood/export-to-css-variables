...
function Generate(options) {
	...
	if (options.useColor) {
		...
		const colors = paintStyles.map((paintStyle) => {
			...
			return {
				name: FixNaming(paintStyle.name, options.scss),
	...
	if (options.useText) {
		...
		const textSizes = textStyles.map((textStyle) => {
			return {
				name: FixNaming(textStyle.name, options.scss),
	...
	if (options.useEffect) {
		...
		const effects = effectStyles.map((effectStyle) => {
			...
			return {
				name: FixNaming(effectStyle.name, options.scss),
				shadows: shadows,
				blur: blur,
			};
		});
...
function FixNaming(name: string, nobem?: boolean) {
	return CamelCaseToKebabCase(
		name
			.trim()
			.replace(/([\/])/g, nobem ? "-" : "--") // Figma uses / to separate different substyles, change this to BEM modifier
